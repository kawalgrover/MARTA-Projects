...
namespace WF1
{
    public partial class ReviewerTaskForm : WebPartPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            btnApprove.Click += new EventHandler(btnApprove_Click);
            btnCancel.Click += new EventHandler(btnCancel_Click);
            btnNoResponse.Click += new EventHandler(btnNoResponse_Click);
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (li[SPBuiltInFieldId.TaskStatus].ToString() == "Expired")
                {
                    pnlTaskNotStarted.Visible = false;
                    pnlTaskExpired.Visible = true;
                }
                else
                {
                    pnlTaskNotStarted.Visible = true;
                    pnlTaskExpired.Visible = false;

                    DateTime dueDate = DateTime.Parse(li["DueDate"].ToString());

                    string assignedToUserName = li["AssignedTo"].ToString();
                    SPFieldUser assignedToField = l.Fields.GetFieldByInternalName("AssignedTo") as SPFieldUser;
                    SPFieldUserValue assignedToValue = (SPFieldUserValue)assignedToField.GetFieldValue(assignedToUserName);
                    SPUser assignedTo = assignedToValue.User;
        ...
        void btnNoResponse_Click(object sender, EventArgs e)
        {
            SPList l = SPContext.Current.List;
            SPListItem li = SPContext.Current.ListItem;
            //li["Body"] = txtComments.Text;
            li[SPBuiltInFieldId.TaskStatus] = "Expired";
            li[SPBuiltInFieldId.PercentComplete] = 0;
